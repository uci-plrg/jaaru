...
uint64_t model_rmw_read_action(void *obj, int atomic_index, const char *position, int size, ModelAction **read_part) {
	createModelIfNotExist();
	*read_part = new ModelAction(ATOMIC_RMWR, position, orders[atomic_index], obj, VALUE_NONE, size);
	return model->switch_to_master(*read_part);
}

ModelAction* model_rmw_action(void *obj, uint64_t val, int atomic_index, const char * position, int size, ModelAction *read_part) {
	createModelIfNotExist();
	ModelAction* action = new ModelAction(ATOMIC_RMW, position, orders[atomic_index], obj, val, size);
	ASSERT(read_part);
	action->setLastWrite(read_part);
	model->switch_to_master(action);
	return action;
}
...
#define _ATOMIC_RMW_(__op__, size, addr, val, atomic_index, position)                                                                   \
	({                                                                                                                              \
		...
		ModelAction *read_part = NULL;														\
		uint ## size ## _t _old = model_rmw_read_action(addr, atomic_index, position, size>>3, &read_part); \
		uint ## size ## _t _copy = _old;                                                                                        \
		uint ## size ## _t _val = val;                                                                                          \
		_copy __op__ _val;                                                                                                      \
		ModelAction *action = model_rmw_action(addr, (uint64_t) _copy, atomic_index, position, size>>3, read_part); \
...
void model_rmw_cas_fail_action(void *obj, int atomic_index, const char *position, int size, ModelAction *read_part) {
	createModelIfNotExist();
	ModelAction *action = new ModelAction(ATOMIC_CAS_FAILED, position, orders[atomic_index], obj, size);
	action->setLastWrite(read_part);
	model->switch_to_master(action);
}
...
#define _ATOMIC_CMPSWP_(size, addr, expected, desired, atomic_index, position)                            \
	({                                                                                              \
		...
		if (_old == _expected) {                                                                    \
			ModelAction *action = model_rmw_action(addr, (uint64_t) _desired, atomic_index, position, size>>3, read_part); \
			...
			*((volatile uint ## size ## _t *)lookupShadowEntry(addr)) = desired;    \
			...
			model_rmw_cas_fail_action(addr, atomic_index, position, size>>3, read_part); _expected = _old; return _old; } \
	})
